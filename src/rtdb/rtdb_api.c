...
int DB_initialization(int _agent, int _second_rtdb) {
    ...
    if (shmem_status.shm_nattch == 1) {
        ...
        for (i = 0; i < p_def[_agent]->n_agents; i++) {
            ...
            if (i == p_def[_agent]->self_agent) {
                ...
                p_def[_agent]->local_mem_size =
                        p_def[_agent]->local_mem_size * 2 + sizeof(TRec) * p_def[_agent]->n_local_recs;
            }
            ...
            p_def[_agent]->shared_mem_size[i] =
                    p_def[_agent]->shared_mem_size[i] * 2 + sizeof(TRec) * p_def[_agent]->n_shared_recs[i];
        }
    }
    ...
    for (i = 0; i < p_def[_agent]->n_agents; i++) {
        ...
        for (j = 0; j < p_def[_agent]->n_shared_recs[i]; j++) {
            ...
            PDEBUG("agent: %d, shared: %d, size: %d, offset:%d, period: %d, lut: %d", i, p_rec->id, p_rec->size,
                   p_rec->offset, p_rec->period, j);
        }
    }
    ...
    for (j = 0; j < p_def[_agent]->n_local_recs; j++) {
        ...
        PDEBUG("local: %d, size: %d, offset:%d, period: %d, lut: %d", p_rec->id, p_rec->size, p_rec->offset,
               p_rec->period, MAX_RECS + j);
...
int DB_put_in(int _agent, int _to_agent, int _id, void *_value, int life) {
    ...
    PDEBUG("agent: %d, id: %d, lut: %d, size: %d, write_bank: %d, previous life: %umsec", _to_agent, p_rec->id, lut,
           p_rec->size, p_rec->read_bank, life);
...
int DB_get_from(int _agent, int _from_agent, int _id, void *_value) {
    ...
    gettimeofday(&time, NULL);
    life = (int) (((time.tv_sec - (p_rec->timestamp[p_rec->read_bank]).tv_sec) * 1E3) +
                  ((time.tv_usec - (p_rec->timestamp[p_rec->read_bank]).tv_usec) / 1E3));

    PDEBUG("agent: %d, from_agent: %d, id: %d, read_bank: %d, life: %umsec", _agent, _from_agent, p_rec->id,
           p_rec->read_bank, life);
